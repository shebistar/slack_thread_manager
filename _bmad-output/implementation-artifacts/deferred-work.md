...
- `role="status"` is semantically overkill for a static version string that never updates — spec-mandated choice; reassess in Story 9.6 WCAG audit.
...
- BriefingPageFrame h1 heading hierarchy and descendant h2 audit — explicitly deferred to Story 9.6 WCAG/heading scope.
- `sticky top-6` sidebar overflow inside `p-6` wrapper on short content — deferred to Story 9.6 responsive/overflow hardening.
