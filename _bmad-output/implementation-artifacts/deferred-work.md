...
- ~~`BriefingPageFrame` uses plain `<div>` wrapper, missing semantic `<header>` landmark~~ — **RESOLVED in Story 9.6**: `<div>` → `<header>` applied to all three page frame header bars (BriefingPageFrame, search.tsx, help.tsx). (`apps/web/src/components/briefing-page-frame/briefing-page-frame.tsx`)
...
## Deferred from: Story 9.6 WCAG audit (2026-07-14)

- **[A1]** Teal context badge fails WCAG AA contrast: `text-[--color-teal-50]` (#37a3a3) on `bg-[--color-teal-10]` (#daf2f2) yields **2.59:1** (AA requires 4.5:1 for normal text at 11px). Affects BriefingPageFrame layout badge ("Filtered Brief", "Lead Architect View", "Executive Scan") and Search page badge. Token established in Story 9.1 design system. **Trigger**: when brand palette is revisited or WCAG compliance is formally required. [`apps/web/src/styles/globals.css`, `apps/web/src/components/briefing-page-frame/briefing-page-frame.tsx`, `apps/web/src/routes/search.tsx`]
- **[A2]** Help version badge marginally fails WCAG AA: `text-[--color-gray-50]` (#707070) on `bg-[--color-gray-10]` (#f2f2f2) yields **4.43:1** (AA threshold 4.5:1). Badge is 11px and purely decorative (displays app version). Main body text (gray-50 on white) passes at 4.96:1. **Trigger**: when brand palette or badge design is revisited. [`apps/web/src/routes/help.tsx`]
- **[A3]** `__APP_VERSION__` and `__APP_CHANGELOG__` silently render as "vundefined"/"undefined" if Vite `define` entry is missing or the dev server is started without the define block — pre-existing risk present since Story 9.4 in `search.tsx`, 9.5 in `app-header.tsx` and `help.tsx`. **Trigger**: when a build-time assertion or fallback (`__APP_VERSION__ ?? '0.0.0'`) is added. [`apps/web/vite.config.ts`, `apps/web/src/components/layout/app-header.tsx`, `apps/web/src/routes/search.tsx`, `apps/web/src/routes/help.tsx`]

## Deferred from: code review of 9-5-global-navigation-header-and-information-architecture-polish (2026-07-12)

- ~~`role="status"` is semantically overkill for a static version string that never updates~~ — **RESOLVED in Story 9.6**: `role="status"` removed from AppHeader version span; `aria-label="App version"` retained.
- `__APP_VERSION__` (and `__APP_CHANGELOG__`) silently render as "vundefined" if the Vite `define` entry is missing — pre-existing risk present in `search.tsx` since Story 9.4; add a build-time assertion or fallback across all usages.
- AppHeader version test regex `/^v/` too permissive — does not verify real semver substitution; tighten to `/^v\d+\.\d+\.\d+/` when adding a semver validation story.
- `-help.test.tsx` module caching and `globalThis.__helpPageComponent` leakage without cleanup — pre-existing pattern shared with `-search.test.tsx`; address when refactoring test isolation strategy.
- NavBar test asserts on raw `className` strings — pre-existing style shared with other nav tests; consider behavior-driven assertions in a future test refactor.
- ~~BriefingPageFrame h1 heading hierarchy and descendant h2 audit~~ — **RESOLVED in Story 9.6**: heading hierarchy audit completed across all routes; all confirmed exactly one h1 with correct semantic nesting.
- ~~`sticky top-6` sidebar overflow inside `p-6` wrapper on short content~~ — **RESOLVED in Story 9.6**: added `max-h-[calc(100vh-7rem)] overflow-y-auto p-1` to Help sidebar nav.
- `isLoading && isError` simultaneously not handled in `FeedLayout` (and other briefing layouts) — pre-existing TanStack Query edge case; low user impact due to retry behaviour.
- Raw `error.message` rendered to users in briefings.tsx error branches — pre-existing security concern; consider sanitizing before Story 9.6 or as a security story.

## Deferred from: code review of 10-5-make-deploy-sh-idempotent-after-install-sh (2026-07-16)

- No `CREATE UNIQUE INDEX CONCURRENTLY` consideration for `uq_silence_alerts_active_thread` — if this migration statement ever re-runs against a live/populated `silence_alerts` table outside a maintenance window, the plain `CREATE UNIQUE INDEX` takes a blocking lock. Pre-existing since migration 0022 was first authored (Epic 7/8); this story only added the `IF NOT EXISTS` guard, not lock behavior. **Trigger**: if silence detection moves to a zero-downtime deploy model. [`packages/db/src/migrations/0022_fair_dreadnoughts.sql:4`]
- Editing an already-shipped migration file (`0022_fair_dreadnoughts.sql`) in place changes its tracked sha256 hash in `migrate-raw.js`'s dedup logic — on any environment where the original 0022 hash was already recorded in `drizzle.__drizzle_migrations`, the next deploy will see a "new" hash and re-run the whole statement batch (now idempotent, so functionally a safe no-op, but it inserts a second tracking row for what is conceptually one migration). Pattern established in prior commit `8584a95`, before this story. **Trigger**: if migration history integrity/auditing becomes a requirement — switch to additive migrations (e.g. `0023_*.sql`) instead of editing shipped files. [`packages/db/src/migrations/0022_fair_dreadnoughts.sql`]
